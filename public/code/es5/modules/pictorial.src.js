...
var PictorialSeries = /** @class */ (function (_super) {
    ...
    PictorialSeries.prototype.pointAttribs = function (point) {
        ...
        if (point && point.shapeArgs && paths) {
            var shape = paths[point.index % paths.length], _a = PictorialSeries_getStackMetrics(series.yAxis, shape), y = _a.y, height = _a.height, pathDef = shape.definition;
            // New pattern, replace
...
PictorialSeries_addEvent(PictorialSeries, 'afterRender', function () {
    ...
    series.points.forEach(function (point) {
        if (point.graphic && point.shapeArgs && paths) {
            var shape = paths[point.index % paths.length], fill = point.graphic.attr('fill'), match = fill && fill.match(fillUrlMatcher), _a = PictorialSeries_getStackMetrics(series.yAxis, shape), y = _a.y, height = _a.height;
...
function renderStackShadow(stack) {
    ...
    if (series &&
        ...
        var xAxis = series.xAxis, options = stack.axis.options, chart = stack.axis.chart, stackShadow = stack.shadow, xCenter = xAxis.toPixels(stack.x, true), x = chart.inverted ? xAxis.len - xCenter : xCenter, paths = series.options.paths || [], index = stack.x % paths.length, shape = paths[index], width = series.getColumnMetrics &&
            series.getColumnMetrics().width, _a = PictorialSeries_getStackMetrics(series.yAxis, shape), height = _a.height, y = _a.y, shadowOptions = options.stackShadow, strokeWidth = PictorialSeries_pick(shadowOptions && shadowOptions.borderWidth, series.options.borderWidth, 1);
        if (!stackShadow &&
            shadowOptions &&
