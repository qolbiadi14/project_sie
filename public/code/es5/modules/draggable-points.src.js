...
function chartSetGuideBoxState(state, options) {
    var guideBox = this.dragGuideBox, guideBoxOptions = merge(DraggablePoints_DragDropDefaults.guideBox, options), stateOptions = merge(guideBoxOptions['default'], // eslint-disable-line dot-notation
...
function mouseUp(e, chart) {
    ...
    if (dragDropData &&
        ...
        var point = dragDropData.point, newPoints = dragDropData.newPoints, numNewPoints = DraggableChart_countProps(newPoints), newPoint = numNewPoints === 1 ?
            DraggableChart_getFirstProp(newPoints) :
            null;
...
function pointGetDropValues(origin, newPos, updateProps) {
    var point = this, series = point.series, chart = series.chart, mapView = chart.mapView, options = DraggablePoints_merge(series.options.dragDrop, point.options.dragDrop), result = {}, pointOrigin = origin.points[point.id], updateSingleProp = Object.keys(updateProps).length === 1;
    /**
...
function pointShowDragHandles() {
    var point = this, series = point.series, chart = series.chart, inverted = chart.inverted, renderer = chart.renderer, options = DraggablePoints_merge(series.options.dragDrop, point.options.dragDrop), dragDropProps = series.dragDropProps || {};
    var dragHandles = chart.dragHandles;
    var _loop_1 = function (key) {
        var val = dragDropProps[key], handleOptions = DraggablePoints_merge(DraggablePoints_DragDropDefaults.dragHandle, val.handleOptions, options.dragHandle), handleAttrs = {
