...
var SonificationInstrument = /** @class */ (function () {
    ...
    SonificationInstrument.prototype.scheduleEventAtTime = function (time, params) {
        var mergedParams = extend(this.curParams, params), freq = SonificationInstrument_defined(params.frequency) ?
...
varLenEnc = function (n) {
    ...
    events.forEach(function (e) {
        var o = e.instrumentEventOptions || {}, t = e.time, dur = cachedDur = MIDI_pick(o.noteDuration, cachedDur), tNOF = dur && e.time + dur, ctrl = [{
...
var SonificationTimeline = /** @class */ (function () {
    ...
    SonificationTimeline.prototype.addChannel = function (type, engine, showPlayMarker, events) {
        ...
        var channel = new Sonification_TimelineChannel(type, engine, showPlayMarker, events);
    ...
    SonificationTimeline.prototype.playClosestToPropValue = function (prop, targetVal, onEnd, onBoundaryHit, eventFilter) {
        ...
        (this.playingChannels || this.channels).forEach(function (channel) {
            ...
            while (i--) {
                ...
                var val = events[i].relatedPoint[prop], diff = SonificationTimeline_defined(val) && Math.abs(targetVal - val);
...
function isActive(context, activeWhen, lastPropValue) {
    ...
    if (typeof activeWhen === 'object') {
        var prop = activeWhen.prop, val = TimelineFromChart_pick(context.value, context.point && getPointPropValue(context.point, prop));
        if (typeof val !== 'number') {
            return false;
        }
        var crossingOk = true;
        ...
        var max = TimelineFromChart_pick(activeWhen.max, Infinity), min = TimelineFromChart_pick(activeWhen.min, -Infinity);
...
function timelineFromChart(audioContext, destinationNode, chart) {
    ...
    chart.series.forEach(function (series, seriesIx) {
        ...
        if (series.visible && sOptions.enabled !== false) {
            var seriesDuration_1 = isSequential ? getAvailableDurationForSeries(series, totalDuration, propMetrics, afterSeriesWait) : totalDuration, seriesDefaultInstrOpts_1 = TimelineFromChart_merge(defaultInstrOpts, sOptions.defaultInstrumentOptions), seriesDefaultSpeechOpts_1 = TimelineFromChart_merge(defaultSpeechOpts, sOptions.defaultSpeechOptions), seriesPointGroupOpts_1 = TimelineFromChart_merge(defaultPointGroupOpts, sOptions.pointGrouping), mainTracks = (sOptions.tracks || [seriesDefaultInstrOpts_1])
...
var Sonification = /** @class */ (function () {
    ...
    Sonification.prototype.playNote = function (instrument, options, delayMs) {
        ...
        var instr = new Sonification_SonificationInstrument(this.audioContext, this.audioDestination, {
