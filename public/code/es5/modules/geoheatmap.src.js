...
var GeoHeatmapSeries = /** @class */ (function (_super) {
    ...
    GeoHeatmapSeries.prototype.drawPoints = function () {
        ...
        if (series.getInterpolation().enabled && mapView && series.bounds) {
            var ctx = series.context || GeoHeatmapSeries_getContext(series), canvas = series.canvas, colorAxis = series.colorAxis, image_1 = series.image, chart_1 = series.chart, points = series.points, _a = [
            ...
            if (canvas && ctx && colorAxis && topLeft && bottomRight) {
                ...
                series.isDirtyCanvas ||
                    ...
                    for (var i = 0; i < points.length; i++) {
                        var p = points[i], sourceArr = new Uint8ClampedArray(GeoHeatmapSeries_colorFromPoint(p.value, p)), _b = p.options, lon = _b.lon, lat = _b.lat;
