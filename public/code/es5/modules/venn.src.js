...
function getMarginFromCircles(point, internal, external) {
    var margin = internal.reduce(function (margin, circle) {
        var m = circle.r - VennUtils_getDistanceBetweenPoints(point, circle);
        return (m <= margin) ? m : margin;
    }, Number.MAX_VALUE);
    margin = external.reduce(function (margin, circle) {
        var m = VennUtils_getDistanceBetweenPoints(point, circle) - circle.r;
        return (m <= margin) ? m : margin;
    }, margin);
    return margin;
}
...
var VennSeries = /** @class */ (function (_super) {
    ...
    VennSeries.getLabelPosition = function (internal, external) {
        ...
        var best = internal.reduce(function (best, circle) {
            ...
            return [
                ...
                var margin = Venn_VennUtils.getMarginFromCircles(point, internal, external);
        ...
        var optimal = Venn_VennUtils.nelderMead(function (p) { return -(Venn_VennUtils.getMarginFromCircles({ x: p[0], y: p[1] }, internal, external)); }, [
            best.x,
            best.y
        ]);
    ...
    VennSeries.getLabelValues = function (relation, setRelations) {
        ...
        var width = Venn_VennUtils.getLabelWidth(position, data.internal, data.external);
        return {
            position: position,
            width: width
        };
    ...
    VennSeries.prototype.translate = function () {
        ...
        var relations = Venn_VennUtils.processVennData(this.options.data, VennSeries.splitter);
        ...
        for (var _i = 0, _b = this.points; _i < _b.length; _i++) {
            var point = _b[_i];
            var sets = VennSeries_isArray(point.sets) ? point.sets : [], id = sets.join(), shape = mapOfIdToShape[id], dataLabelValues = mapOfIdToLabelValues[id] || {}, dlOptions = point.options && point.options.dataLabels;
