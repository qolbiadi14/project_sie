...
var ColorAxis = /** @class */ (function (_super) {
    ...
    ColorAxis.prototype.setOptions = function (userOptions) {
        var options = ColorAxis_merge(defaultOptions.colorAxis, userOptions, 
        // Forced options
        {
            showEmpty: false,
            title: null,
    ...
    ColorAxis.prototype.drawLegendSymbol = function (legend, item) {
        var _a;
        var axis = this, legendItem = item.legendItem || {}, padding = legend.padding, legendOptions = legend.options, labelOptions = axis.options.labels, itemDistance = ColorAxis_pick(legendOptions.itemDistance, 10), horiz = axis.horiz, _b = axis.getSize(), width = _b.width, height = _b.height, labelPadding = ColorAxis_pick(
    ...
    ColorAxis.prototype.getSize = function () {
        var axis = this, chart = axis.chart, horiz = axis.horiz, _a = axis.options, colorAxisHeight = _a.height, colorAxisWidth = _a.width, legendOptions = chart.options.legend, width = ColorAxis_pick(defined(colorAxisWidth) ?
            relativeLength(colorAxisWidth, chart.chartWidth) : void 0, legendOptions === null || legendOptions === void 0 ? void 0 : legendOptions.symbolWidth, horiz ? ColorAxis.defaultLegendLength : 12), height = ColorAxis_pick(defined(colorAxisHeight) ?
        ...
        if (!chart.renderer.forExport &&
            ...
            objectEach(navOptions.buttons, function (buttonOptions, n) {
                ...
                if (!chart.hasLoaded) {
                    ...
                    var unbind_1 = MapNavigation_addEvent(chart, 'load', function () {
...
var MapPoint = /** @class */ (function (_super) {
    ...
    MapPoint.prototype.getProjectedBounds = function (projection) {
        var path = MapPoint.getProjectedPath(this, projection), bounds = MapPoint_boundsFromPath(path), properties = this.properties, mapView = this.series.chart.mapView;
        if (bounds) {
            ...
            if (mapView && MapPoint_isNumber(propMiddleLon) && MapPoint_isNumber(propMiddleLat)) {
                ...
                var middleYFraction = MapPoint_pick(this.middleY, MapPoint_isNumber(propMiddleY) ? propMiddleY : 0.5);
...
var LambertConformalConic = /** @class */ (function () {
    ...
    function LambertConformalConic(options) {
        ...
        var parallels = (options.parallels || [])
            .map(function (n) { return n * LambertConformalConic_deg2rad; }), lat1 = parallels[0] || 0, lat2 = (_a = parallels[1]) !== null && _a !== void 0 ? _a : lat1, cosLat1 = Math.cos(lat1);
    ...
    LambertConformalConic.prototype.forward = function (lonLat) {
        var _a = this, c = _a.c, n = _a.n, projectedBounds = _a.projectedBounds, lon = lonLat[0] * LambertConformalConic_deg2rad;
...
var Orthographic = /** @class */ (function () {
    ...
    Orthographic.prototype.forward = function (lonLat) {
        var lonDeg = lonLat[0], latDeg = lonLat[1], lat = latDeg * Orthographic_deg2rad, xy = [
...
var havFromCoords = function (point1, point2) {
    var cos = Math.cos, lat1 = point1[1] * Projection_deg2rad, lon1 = point1[0] * Projection_deg2rad, lat2 = point2[1] * Projection_deg2rad, lon2 = point2[0] * Projection_deg2rad, deltaLat = lat2 - lat1, deltaLon = lon2 - lon1, havFromCoords = hav(deltaLat) + cos(lat1) * cos(lat2) * hav(deltaLon);
...
var Projection = /** @class */ (function () {
    ...
    Projection.prototype.getRotator = function (rotation) {
        var deltaLambda = rotation[0] * Projection_deg2rad, deltaPhi = (rotation[1] || 0) * Projection_deg2rad, deltaGamma = (rotation[2] || 0) * Projection_deg2rad;
        ...
        return {
            forward: function (lonLat) {
                ...
                var lat = lonLat[1] * Projection_deg2rad, cosLat = Math.cos(lat), x = Math.cos(lon) * cosLat, y = Math.sin(lon) * cosLat, sinLat = Math.sin(lat), k = sinLat * cosDeltaPhi + x * sinDeltaPhi;
                return [
            ...
            inverse: function (rLonLat) {
                ...
                var lat = rLonLat[1] * Projection_deg2rad, cosLat = Math.cos(lat), x = Math.cos(lon) * cosLat, y = Math.sin(lon) * cosLat, sinLat = Math.sin(lat), k = sinLat * cosDeltaGamma - y * sinDeltaGamma;
                return [
            ...
            if (isPolygon) {
                ...
                while (i >= 0) {
                    ...
                    var lonMinus = wrapLon(antimeridian -
                        intersections[i].direction * floatCorrection);
                    var slice = poly.splice.apply(poly, Projection_spreadArray([index,
...
var MapView = /** @class */ (function () {
    ...
    function MapView(chart, options) {
        ...
        var o = MapView_merge(Maps_MapViewDefaults, this.recommendedMapView, options);
    ...
    MapView.mergeInsets = function (a, b) {
        ...
        var insetsObj = MapView_merge(toObject(a), toObject(b)), insets = Object
            .keys(insetsObj)
    ...
    MapView.prototype.createInsets = function () {
        ...
        if (insets) {
            insets.forEach(function (item) {
                var inset = new MapViewInset(_this, MapView_merge(options.insetOptions, item));
    ...
    MapView.prototype.fitToBounds = function (bounds, padding, redraw, animation) {
        ...
        if (b) {
            var pad = MapView_pick(padding, bounds ? 0 : this.options.padding), fullField = this.getField(false), padArr = MapView_isArray(pad) ? pad : [pad, pad, pad, pad];
            this.padding = [
    ...
    MapView.prototype.update = function (options, redraw, animation) {
        if (redraw === void 0) { redraw = true; }
        var newProjection = options.projection, isDirtyProjection = newProjection && ((Maps_Projection.toString(newProjection) !==
        ...
        if (coordinates) {
            ...
            if (options.units === 'percent') {
                ...
                polygon = polygon.map(function (xy) { return [
                    MapView_relativeLength("".concat(xy[0], "%"), relativeTo_1.width, relativeTo_1.x),
                    MapView_relativeLength("".concat(xy[1], "%"), relativeTo_1.height, relativeTo_1.y)
                ]; });
            }
            return {
                type: 'Polygon',
        ...
        if (borderPath && mapView.group) {
            ...
            var d = (borderPath.coordinates || []).reduce(function (d, lineString) {
                return lineString.reduce(function (d, point, i) {
                    ...
                    if (options.units === 'percent') {
                        x = chart.plotLeft + MapView_relativeLength("".concat(x, "%"), field_1.width, field_1.x);
                        y = chart.plotTop + MapView_relativeLength("".concat(y, "%"), field_1.height, field_1.y);
                    }
...
var MapSeries = /** @class */ (function (_super) {
    ...
    MapSeries.prototype.drawPoints = function () {
        ...
        transformGroups.forEach(function (transformGroup, i) {
            var view = i === 0 ? mapView : mapView.insets[i - 1], svgTransform = view.getSVGTransform(), strokeWidth = MapSeries_pick(_this.getStrokeWidth(_this.options), 1 // Styled mode
            );
            /*
            ...
            */
            ...
            if (renderer.globalAnimation &&
                ...
                var animOptions = MapSeries_merge(animObject(renderer.globalAnimation)), userStep_1 = animOptions.step;
...
var BubbleLegendItem = /** @class */ (function () {
    ...
    BubbleLegendItem.prototype.drawLegendSymbol = function (legend) {
        var itemDistance = BubbleLegendItem_pick(legend.options.itemDistance, 20), legendItem = this.legendItem || {}, options = this.options, ranges = options.ranges, connectorDistance = options.connectorDistance;
        var connectorSpace;
...
var BubbleSeries = /** @class */ (function (_super) {
    ...
    BubbleSeries.prototype.pointAttribs = function (point, state) {
        var markerOptions = this.options.marker, fillOpacity = markerOptions.fillOpacity, attr = BubbleSeries_Series.prototype.pointAttribs.call(this, point, state);
        if (fillOpacity !== 1) {
    ...
    BubbleSeries.prototype.getPxExtremes = function () {
        ...
        // always visible. #5873
        var maxPxSize = Math.max(getPxSize(BubbleSeries_pick(this.options.maxSize, '20%')), minPxSize);
        return { minPxSize: minPxSize, maxPxSize: maxPxSize };
    };
    BubbleSeries.prototype.getZExtremes = function () {
        var options = this.options, zData = this.getColumn('z').filter(BubbleSeries_isNumber);
        if (zData.length) {
            var zMin = BubbleSeries_pick(options.zMin, BubbleSeries_clamp(BubbleSeries_arrayMin(zData), options.displayNegative === false ?
                (options.zThreshold || 0) :
                -Number.MAX_VALUE, Number.MAX_VALUE));
            var zMax = BubbleSeries_pick(options.zMax, BubbleSeries_arrayMax(zData));
...
var HeatmapSeries = /** @class */ (function (_super) {
    ...
    HeatmapSeries.prototype.drawPoints = function () {
        ...
        if (interpolation) {
            ...
            if (!image || series.isDirtyData || series.isDirtyCanvas) {
                var ctx = HeatmapSeries_getContext(series), canvas = series.canvas, _c = series.options, _d = _c.colsize, colsize = _d === void 0 ? 1 : _d, _e = _c.rowsize, rowsize = _e === void 0 ? 1 : _e, points = series.points, length_1 = series.points.length, pointsLen = length_1 - 1, colorAxis = (chart.colorAxis && chart.colorAxis[0]);
    ...
    HeatmapSeries.prototype.pointAttribs = function (point, state) {
        var series = this, attr = HeatmapSeries_Series.prototype.pointAttribs.call(series, point, state), seriesOptions = series.options || {}, plotOptions = series.chart.options.plotOptions || {}, seriesPlotOptions = plotOptions.series || {}, heatmapPlotOptions = plotOptions.heatmap || {}, 
