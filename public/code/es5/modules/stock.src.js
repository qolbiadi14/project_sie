...
var NavigatorAxisAdditions = /** @class */ (function () {
    ...
    NavigatorAxisAdditions.prototype.toFixedRange = function (pxMin, pxMax, fixedMin, fixedMax) {
        ...
        var newMin = NavigatorAxisComposition_pick(fixedMin, axis.translate(pxMin, true, !axis.horiz)), newMax = NavigatorAxisComposition_pick(fixedMax, axis.translate(pxMax, true, !axis.horiz));
...
function arc(cx, cy, w, h, options) {
    var arc = [];
    if (options) {
        var start = options.start || 0, rx = Symbols_pick(options.r, w), ry = Symbols_pick(options.r, h || w), 
        ...
        proximity = 0.0002 / (options.borderRadius ? 1 : Math.max(rx, 1)), fullCircle = (Math.abs((options.end || 0) - start - 2 * Math.PI) <
            proximity), end = (options.end || 0) - (fullCircle ? proximity : 0), innerRadius = options.innerR, open_1 = Symbols_pick(options.open, fullCircle), cosStart = Math.cos(start), sinStart = Math.sin(start), cosEnd = Math.cos(end), sinEnd = Math.sin(end), 
...
(function (ScrollbarAxis) {
    ...
    function getExtremes(axis) {
        var axisMin = ScrollbarAxis_pick(axis.options && axis.options.min, axis.min);
        var axisMax = ScrollbarAxis_pick(axis.options && axis.options.max, axis.max);
        return {
            axisMin: axisMin,
            axisMax: axisMax,
...
function numExt(extreme) {
    ...
    var numbers = [].filter.call(args, Navigator_isNumber);
    if (numbers.length) {
...
var Navigator = /** @class */ (function () {
    ...
    Navigator.prototype.render = function (min, max, pxMin, pxMax) {
        ...
        var newMin = xAxis.toValue(pxMin, true), newMax = xAxis.toValue(pxMax, true), currentRange = Math.abs(Navigator_correctFloat(newMax - newMin));
    ...
    Navigator.prototype.updateNavigatorSeries = function (addEvents, redraw) {
        ...
        if (baseSeries && baseSeries.length) {
            baseSeries.forEach(function (base) {
                var _a;
                var linkedNavSeries = base.navigatorSeries, userNavOptions = Navigator_extend(
    ...
    Navigator.prototype.modifyBaseAxisExtremes = function () {
        var _a;
        var baseXAxis = this, navigator = baseXAxis.chart.navigator, baseExtremes = baseXAxis.getExtremes(), baseMin = baseExtremes.min, baseMax = baseExtremes.max, baseDataMin = baseExtremes.dataMin, baseDataMax = baseExtremes.dataMax, range = baseMax - baseMin, stickToMin = navigator.stickToMin, stickToMax = navigator.stickToMax, overscroll = Navigator_pick((_a = baseXAxis.ordinal) === null || _a === void 0 ? void 0 : _a.convertOverscroll(baseXAxis.options.overscroll), 0), navigatorSeries = navigator.series && navigator.series[0], hasSetExtremes = !!baseXAxis.setExtremes, 
...
function axisMinFromRange() {
    ...
    var dataMin = RangeSelectorComposition_pick(this.dataMin, Number.MIN_VALUE);
...
var RangeSelector = /** @class */ (function () {
    ...
    RangeSelector.prototype.clickButton = function (i, redraw) {
        ...
        var dataMin = unionExtremes.dataMin, dataMax = unionExtremes.dataMax, newMin, newMax = RangeSelector_isNumber(baseAxis === null || baseAxis === void 0 ? void 0 : baseAxis.max) ? Math.round(Math.min(baseAxis.max, dataMax !== null && dataMax !== void 0 ? dataMax : baseAxis.max)) : void 0, // #1568
        ...
        if (!baseAxis) {
            ...
            var axisRangeUpdateEvent_1 = RangeSelector_addEvent(chart, 'afterCreateAxes', function () {
    ...
    RangeSelector.prototype.updateButtonStates = function () {
        ...
        rangeSelector.buttonOptions.forEach(function (rangeOptions, i) {
            ...
            if (baseAxis.isOrdinal &&
                ...
                var positions = baseAxis.ordinal.positions, prevOrdinalPosition = Axis_OrdinalAxis.Additions.findIndexOf(positions, baseAxis.min, true), nextOrdinalPosition = Math.min(Axis_OrdinalAxis.Additions.findIndexOf(positions, baseAxis.max, true) + 1, positions.length - 1);
    ...
    RangeSelector.prototype.drawInput = function (name) {
        ...
        var rangeSelector = this, chartStyle = chart.renderer.style || {}, renderer = chart.renderer, options = chart.options.rangeSelector, lang = RangeSelector_defaultOptions.lang, isMin = name === 'min';
        /**
         * @private
         */
    ...
    RangeSelector.prototype.createElements = function () {
        var _a;
        var chart = this.chart, renderer = chart.renderer, container = chart.container, chartOptions = chart.options, options = chartOptions.rangeSelector, inputEnabled = options.inputEnabled, inputsZIndex = RangeSelector_pick((_a = chartOptions.chart.style) === null || _a === void 0 ? void 0 : _a.zIndex, 0) + 1;
...
function getForcedAxisOptions(type, chartOptions) {
    if (type === 'xAxis') {
        ...
        // is enabled (#1090)
        var navigatorEnabled = StockChart_pick(chartOptions.navigator && chartOptions.navigator.enabled, Navigator_NavigatorDefaults.enabled, true);
        var axisOptions = {
            type: 'datetime',
            categories: void 0
        };
        ...
        (chart.options.isStock && e.acrossPanes !== false) &&
            ...
            for (var _i = 0, axes2_1 = axes2; _i < axes2_1.length; _i++) {
                ...
                if (!A.options.isInternal) {
                    var a = (A.isXAxis ? 'yAxis' : 'xAxis'), relatedAxis = (StockChart_defined(A.options[a]) ?
                        chart[a][A.options[a]] :
                        chart[a][0]);
                    if (axis === relatedAxis) {
                        axes.push(A);
            ...
            var _loop_1 = function (axis2) {
                if (uniqueAxes.indexOf(axis2) === -1 &&
                    ...
                    !StockChart_find(uniqueAxes, function (unique) { return (unique.pos === axis2.pos &&
...
var HLCSeries = /** @class */ (function (_super) {
    ...
    HLCSeries.prototype.getPointPath = function (point, graphic) {
        ...
        // Close
        if (point.close !== null) {
            var plotClose = HLCSeries_crisp(point.plotClose, strokeWidth);
...
var OHLCSeries = /** @class */ (function (_super) {
    ...
    OHLCSeries.prototype.getPointPath = function (point, graphic) {
        var path = _super.prototype.getPointPath.call(this, point, graphic), strokeWidth = graphic.strokeWidth(), crispX = OHLCSeries_crisp(point.plotX || 0, strokeWidth), halfWidth = Math.round(point.shapeArgs.width / 2);
        if (point.open !== null) {
            var plotOpen = OHLCSeries_crisp(point.plotOpen, strokeWidth);
...
var CandlestickSeries = /** @class */ (function (_super) {
    ...
    CandlestickSeries.prototype.pointAttribs = function (point, state) {
        var attribs = CandlestickSeries_ColumnSeries.prototype.pointAttribs.call(this, point, state), options = this.options, isUp = point.open < point.close, stroke = options.lineColor || this.color, color = point.color || this.color; // (#14826)
...
var FlagsSeries = /** @class */ (function (_super) {
    ...
    FlagsSeries.prototype.drawPoints = function () {
        var _a, _b;
        var series = this, points = series.points, chart = series.chart, renderer = chart.renderer, inverted = chart.inverted, options = series.options, optionsY = options.y, yAxis = series.yAxis, boxesMap = {}, boxes = [], borderRadius = FlagsSeries_isNumber(options.borderRadius) ?
...
(function (BrokenAxis) {
    ...
    var Additions = /** @class */ (function () {
        ...
        Additions.prototype.setBreaks = function (breaks, redraw) {
            var brokenAxis = this, axis = brokenAxis.axis, time = axis.chart.time, hasBreaks = BrokenAxis_isArray(breaks) &&
                !!breaks.length &&
            ...
            if (hasBreaks) {
                ...
                axis.setAxisTranslation = function () {
                    ...
                    if (brokenAxis.hasBreaks) {
                        ...
                        // Temporary one:
                        breakArrayT_1 = [], breakArray_1 = [], pointRangePadding = BrokenAxis_pick(axis.pointRangePadding, 0);
...
function onTooltipHeaderFormatter(e) {
    ...
    if (xAxis &&
        ...
        var groupStart = DataGrouping_pick((_a = series.groupMap) === null || _a === void 0 ? void 0 : _a[point.index].groupStart, point.key), groupEnd = groupStart + ((currentDataGrouping === null || currentDataGrouping === void 0 ? void 0 : currentDataGrouping.totalRange) || 0) - 1;
...
var zoomBy = function (chart, howMuch, xAxis, yAxis, mouseX, mouseY, options) {
    var type = MouseWheelZoom_pick(options.type, chart.zooming.type, '');
    var axes = [];
    if (type === 'x') {
        axes = xAxis;
    }
    ...
    if (wheelZoomOptions.enabled) {
        MouseWheelZoom_addEvent(this.container, 'wheel', function (e) {
            ...
            if (_this.isInsidePlot(e.chartX - _this.plotLeft, e.chartY - _this.plotTop) && allowZoom) {
                var wheelSensitivity = wheelZoomOptions.sensitivity || 1.1, delta = e.detail || ((e.deltaY || 0) / 120), xAxisCoords = MouseWheelZoom_getAssignedAxis(pointer.getCoordinates(e).xAxis), yAxisCoords = MouseWheelZoom_getAssignedAxis(pointer.getCoordinates(e).yAxis);
