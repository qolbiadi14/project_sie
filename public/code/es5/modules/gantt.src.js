...
var simpleConnect = function (start, end, options) {
    ...
    var endSegment, dir = PathfinderAlgorithms_pick(options.startDirectionX, abs(end.x - start.x) > abs(end.y - start.y)) ? 'x' : 'y', startObstacle, endObstacle, waypoint, useMax, endPoint;
    ...
    var path = Series_PathUtilities.applyRadius(pathFromSegments(segments), options.radius);
    return {
        path: path,
        obstacles: segments
    };
...
function fastAvoid(start, end, options) {
    ...
    */
    var dirIsX = PathfinderAlgorithms_pick(options.startDirectionX, abs(end.x - start.x) > abs(end.y - start.y)), dir = dirIsX ? 'x' : 'y', endSegments = [], 
    ...
    function extractFromObstacle(obstacle, point, goalPoint) {
        var dirIsX = PathfinderAlgorithms_min(obstacle.xMax - point.x, point.x - obstacle.xMin) <
...
function calculateObstacleDistance(a, b, bbMargin) {
    ...
    var margin = Pathfinder_pick(bbMargin, 10), yOverlap = a.yMax + margin > b.yMin - margin &&
...
var Pathfinder = /** @class */ (function () {
    ...
    * @return {Array<object>}
    ...
    Pathfinder.prototype.getChartObstacles = function (options) {
        var series = this.chart.series, margin = Pathfinder_pick(options.algorithmMargin, 0);
    ...
    if (chart.redrawTrigger !== 'adjustHeight') {
        var _loop_1 = function (axis) {
            ...
            if (axis.staticScale && StaticScale_defined(axis.min)) {
                var height = StaticScale_pick(axis.brokenAxis && axis.brokenAxis.unitLength, axis.max + axis.tickInterval - axis.min) * staticScale;
...
var XRangeSeries = /** @class */ (function (_super) {
    ...
    XRangeSeries.prototype.findPointIndex = function (options) {
        ...
        var pointIndex;
        if (id) {
            var point = XRangeSeries_find(points, function (point) { return point.id === id; });
            pointIndex = point ? point.index : void 0;
        }
        if (typeof pointIndex === 'undefined') {
            var point = XRangeSeries_find(points, function (point) { return (point.x === options.x &&
                point.x2 === options.x2 &&
                !point.touched); });
            pointIndex = point ? point.index : void 0;
        }
    ...
    XRangeSeries.prototype.translatePoint = function (point) {
        ...
        var length = Math.abs(plotX2 - plotX), inverted = this.chart.inverted, borderWidth = XRangeSeries_pick(options.borderWidth, 1);
        ...
        var x = XRangeSeries_crisp(Math.min(plotX, plotX2), borderWidth), x2 = XRangeSeries_crisp(Math.max(plotX, plotX2), borderWidth), width = x2 - x;
...
function onAfterInit() {
    ...
    if (gridOptions.columns) {
        ...
        while (++columnIndex < gridOptions.columns.length) {
            var columnOptions = GridAxis_merge(userOptions, gridOptions.columns[columnIndex], {
                isInternal: true,
                linkedTo: 0,
...
function onAfterSetOptions(e) {
    var options = this.options, userOptions = e.userOptions, gridOptions = ((options && GridAxis_isObject(options.grid)) ? options.grid : {});
    var gridAxisOptions;
    if (gridOptions.enabled === true) {
