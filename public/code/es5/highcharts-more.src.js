...
var AreaRangeSeries = /** @class */ (function (_super) {
    ...
    AreaRangeSeries.prototype.getGraphPath = function (points) {
        var highPoints = [], highAreaPoints = [], getGraphPath = AreaRangeSeries_areaProto.getGraphPath, options = this.options, polar = this.chart.polar, connectEnds = polar && options.connectEnds !== false, connectNulls = options.connectNulls;
...
var BoxPlotSeries = /** @class */ (function (_super) {
    ...
    BoxPlotSeries.prototype.getWhiskerPair = function (halfWidth, stemX, upperWhiskerLength, lowerWhiskerLength, point) {
        var strokeWidth = point.whiskers.strokeWidth(), getWhisker = function (xLen, yPos) {
            var halfLen = BoxPlotSeries_relativeLength(xLen, 2 * halfWidth) / 2, crispedYPos = crisp(yPos, strokeWidth);
            return [
                [
                    'M',
                    crisp(stemX - halfLen),
...
var BubbleSeries = /** @class */ (function (_super) {
    ...
    BubbleSeries.prototype.getZExtremes = function () {
        ...
        if (zData.length) {
            var zMin = BubbleSeries_pick(options.zMin, clamp(BubbleSeries_arrayMin(zData), options.displayNegative === false ?
...
var ColumnPyramidSeries = /** @class */ (function (_super) {
    ...
    ColumnPyramidSeries.prototype.translate = function () {
        ...
        for (var _i = 0, _a = series.points; _i < _a.length; _i++) {
            var point = _a[_i];
            var yBottom = ColumnPyramidSeries_pick(point.yBottom, translatedThreshold), safeDistance = 999 + Math.abs(yBottom), plotY = ColumnPyramidSeries_clamp(point.plotY, -safeDistance, yAxis.len + safeDistance), 
...
var GaugeSeries = /** @class */ (function (_super) {
    ...
    GaugeSeries.prototype.drawPoints = function () {
        ...
        series.points.forEach(function (point) {
            var graphic = point.graphic, shapeArgs = point.shapeArgs, d = shapeArgs.d, dialOptions = GaugeSeries_merge(options.dial, point.dial); // #1233
            if (graphic) {
...
function setPolygon(event) {
    ...
    if (tp) {
        ...
        var appendTopAndBottom = function (charIndex, positionOfChar) {
            var x = positionOfChar.x, y = positionOfChar.y, rotation = (tp.getRotationOfChar(charIndex) - 90) * TextPath_deg2rad, cosRot = Math.cos(rotation), sinRot = Math.sin(rotation);
            return [
                [
...
(function (RadialAxis) {
    ...
    function getLinePath(_lineWidth, radius, innerRadius) {
        ...
        var end, r = RadialAxis_pick(radius, center[2] / 2 - this.offset), path;
    ...
    function getPlotBandPath(from, to, options) {
        ...
        var start, end, angle, xOnPerimeter, open, path, outerRadius = RadialAxis_pick(radiusToPixels(options.outerRadius), fullRadius), innerRadius = radiusToPixels(options.innerRadius), thickness = RadialAxis_pick(radiusToPixels(options.thickness), 10);
    ...
    function setOptions(userOptions) {
        ...
        var options = this.options = RadialAxis_merge(defaultPolarOptions, userOptions);
...
function clipCircle(renderer, x, y, r, innerR) {
    var id = PolarComposition_uniqueKey(), clipPath = renderer.createElement('clipPath').attr({
        id: id
...
function trimPath(path, start, end, radialAxis) {
    ...
    var lastTick = find(ticks, function (tick) { return tick >= end; }), firstTick = find(PolarComposition_spreadArray([], ticks, true).reverse(), function (tick) { return tick <= start; });
...
function wrapColumnSeriesAlignDataLabel(proceed, point, dataLabel, options, alignTo, isNew) {
    var chart = this.chart, inside = PolarComposition_pick(options.inside, !!this.options.stacking);
...
var PolarAdditions = /** @class */ (function () {
    ...
    PolarAdditions.prototype.arc = function (low, high, start, end) {
        ...
        var r = len - high + paneInnerR, innerR = len - PolarComposition_pick(low, len) + paneInnerR;
...
(function (WaterfallAxis) {
    ...
    var Composition = /** @class */ (function () {
        ...
        Composition.prototype.renderStackTotals = function () {
            var yAxis = this.axis, waterfallStacks = yAxis.waterfall.stacks, stackTotalGroup = (yAxis.stacking && yAxis.stacking.stackTotalGroup), dummyStackItem = new (highcharts_StackItem_commonjs_highcharts_StackItem_commonjs2_highcharts_StackItem_root_Highcharts_StackItem_default())(yAxis, yAxis.options.stackLabels || {}, false, 0, void 0);
...
var WaterfallSeries = /** @class */ (function (_super) {
    ...
    WaterfallSeries.prototype.pointAttribs = function (point, state) {
        ...
        var attr = WaterfallSeries_ColumnSeries.prototype.pointAttribs.call(this, point, state);
    ...
    WaterfallSeries.prototype.getCrispPath = function () {
        ...
        data = this.data.filter(function (d) { return WaterfallSeries_isNumber(d.y); }), yAxis = this.yAxis, length = data.length, graphLineWidth = ((_a = this.graph) === null || _a === void 0 ? void 0 : _a.strokeWidth()) || 0, reversedXAxis = this.xAxis.reversed, reversedYAxis = this.yAxis.reversed, stacking = this.options.stacking, path = [];
...
WaterfallSeries_addEvent(WaterfallSeries, 'afterColumnTranslate', function () {
    var series = this, options = series.options, points = series.points, yAxis = series.yAxis, minPointLength = WaterfallSeries_pick(options.minPointLength, 5), halfMinPointLength = minPointLength / 2, threshold = options.threshold || 0, stacking = options.stacking, actualStack = yAxis.waterfall.stacks[series.stackKey], processedYData = series.getColumn('y', true);
    var previousIntermediate = threshold, previousY = threshold, y, total, yPos, hPos;
    for (var i = 0; i < points.length; i++) {
        var point = points[i], yValue = processedYData[i], shapeArgs = point.shapeArgs, box = WaterfallSeries_extend({
            x: 0,
            y: 0,
            width: 0,
            height: 0
        ...
        var crispBottom = WaterfallSeries_crisp(point.yBottom, series.borderWidth);
